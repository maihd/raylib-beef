...
static
{
	...
	public static mixin MatrixToFloat(var mat)  
	{
		(MatrixToFloatV(mat).v)
	}
	
	// Get float vector for Vector3
	public static mixin Vector3ToFloat(var vec)
	{
	...
	{
	    let result = (Math.Abs(x - y)) <= (EPSILON * Math.Max(1.0f, Math.Max(Math.Abs(x), Math.Abs(y))));
	    return result;
	}
	...
	{
	    float result = Math.Atan2(v2.y - v1.y, v2.x - v1.x);
    ...
    public static float Vector2LineAngle(Vector2 start, Vector2 end)
	{
	    ...
	    result = Math.Acos(dotClamp);
	
	    // Alternative implementation, more costly
	    //float v1Length = Math.Sqrt((start.x*start.x) + (start.y*start.y));
	    //float v2Length = Math.Sqrt((end.x*end.x) + (end.y*end.y));
	    //float result = -Math.Acos((start.x*end.x + start.y*end.y)/(v1Length*v2Length));
	...
	[Inline] 
    public static bool Vector2Equals(Vector2 p, Vector2 q)
	{
	    let result =  ((Math.Abs(p.x - q.x)) <= (EPSILON*Math.Max(1.0f, Math.Max(Math.Abs(p.x), Math.Abs(q.x))))) &&
	...
	// Check whether two given vectors are almost equal
	[Inline] 
    public static bool Vector3Equals(Vector3 p, Vector3 q)
	{
