...
*
*       static uint32 guiStyle[RAYGUI_MAX_CONTROLS*(RAYGUI_MAX_PROPS_BASE + RAYGUI_MAX_PROPS_EXTENDED)];
*
...
*
*       static uint32 guiIcons[RAYGUI_ICON_MAX_ICONS*RAYGUI_ICON_DATA_ELEMENTS];
*
...
static
{
	const char8* RAYGUI_VERSION = "3.2";
}
...
static
{
	static mixin RAYGUI_MALLOC(var sz)
	{
		Internal.StdMalloc(sz)	
	}

	static mixin RAYGUI_CALLOC(var n, var sz)
	{
		Internal.StdMalloc(n * sz)
	}

	static mixin RAYGUI_FREE(var p)
	{
		Internal.StdFree(p)
	}
}
...
static
{
    const uint32 SCROLLBAR_LEFT_SIDE     = 0;
    const uint32 SCROLLBAR_RIGHT_SIDE    = 1;
}
...
static
{
    ...
    /*public static void GuiEnable();                                         // Enable gui controls (global state)
    public static void GuiDisable();                                        // Disable gui controls (global state)
    public static void GuiLock();                                           // Lock gui controls (global state)
    public static void GuiUnlock();                                         // Unlock gui controls (global state)
    public static bool GuiIsLocked();                                       // Check if gui is locked (global state)
    public static void GuiFade(float alpha);                                    // Set gui controls alpha (global state), alpha goes from 0.0f to 1.0f
    public static void GuiSetState(int state);                                  // Set gui state (global state)
    public static int GuiGetState();                                        // Get gui state (global state)*/

    // Font set/get functions
    /*public static void GuiSetFont(Font font);                                   // Set gui custom font (global state)
    public static Font GuiGetFont();                                        // Get gui custom font (global state)*/

    // Style set/get functions
    /*public static void GuiSetStyle(int control, int property, int value);       // Set one style property
    public static int GuiGetStyle(int control, int property);                   // Get one style property*/

    // Container/separator controls, useful for controls organization
    /*public static bool GuiWindowBox(Rectangle bounds, char8* title);                                       // Window Box control, shows a window that can be closed
    public static void GuiGroupBox(Rectangle bounds, char8* text);                                         // Group Box control with text name
    public static void GuiLine(Rectangle bounds, char8* text);                                             // Line separator control, could contain text
    public static void GuiPanel(Rectangle bounds, char8* text);                                            // Panel control, useful to group controls
    public static Rectangle GuiScrollPanel(Rectangle bounds, char8* text, Rectangle content, Vector2 *scroll); // Scroll Panel control*/

    // Basic controls set
    /*public static void GuiLabel(Rectangle bounds, char8* text);                                            // Label control, shows text
    public static bool GuiButton(Rectangle bounds, char8* text);                                           // Button control, returns true when clicked
    public static bool GuiLabelButton(Rectangle bounds, char8* text);                                      // Label button control, show true when clicked
    public static bool GuiToggle(Rectangle bounds, char8* text, bool active);                              // Toggle Button control, returns true when active
    public static int GuiToggleGroup(Rectangle bounds, char8* text, int active);                           // Toggle Group control, returns active toggle index
    public static bool GuiCheckBox(Rectangle bounds, char8* text, bool @checked);                           // Check Box control, returns true when active
    public static int GuiComboBox(Rectangle bounds, char8* text, int active);                              // Combo Box control, returns selected item index
    public static bool GuiDropdownBox(Rectangle bounds, char8* text, int *active, bool editMode);          // Dropdown Box control, returns selected item
    public static bool GuiSpinner(Rectangle bounds, char8* text, int *value, int minValue, int maxValue, bool editMode);     // Spinner control, returns selected value
    public static bool GuiValueBox(Rectangle bounds, char8* text, int *value, int minValue, int maxValue, bool editMode);    // Value Box control, updates input text with numbers
    public static bool GuiTextBox(Rectangle bounds, char8* text, int textSize, bool editMode);                   // Text Box control, updates input text
    public static bool GuiTextBoxMulti(Rectangle bounds, char8* text, int textSize, bool editMode);              // Text Box control with multiple lines
    public static float GuiSlider(Rectangle bounds, char8* textLeft, char8* textRight, float value, float minValue, float maxValue);       // Slider control, returns selected value
    public static float GuiSliderBar(Rectangle bounds, char8* textLeft, char8* textRight, float value, float minValue, float maxValue);    // Slider Bar control, returns selected value
    public static float GuiProgressBar(Rectangle bounds, char8* textLeft, char8* textRight, float value, float minValue, float maxValue);  // Progress Bar control, shows current progress value
    public static void GuiStatusBar(Rectangle bounds, char8* text);                                        // Status Bar control, shows info text
    public static void GuiDummyRec(Rectangle bounds, char8* text);                                         // Dummy control for placeholders
    public static Vector2 GuiGrid(Rectangle bounds, char8* text, float spacing, int subdivs);              // Grid control, returns mouse cell position*/

    // Advance controls set
    /*public static int GuiListView(Rectangle bounds, char8* text, int *scrollIndex, int active);            // List View control, returns selected list item index
    public static int GuiListViewEx(Rectangle bounds, char8* *text, int count, int *focus, int *scrollIndex, int active);      // List View with extended parameters
    public static int GuiMessageBox(Rectangle bounds, char8* title, char8* message, char8* buttons);                 // Message Box control, displays a message
    public static int GuiTextInputBox(Rectangle bounds, char8* title, char8* message, char8* buttons, char8* text, int textMaxSize, int *secretViewActive);   // Text Input Box control, ask for text, supports secret
    public static Color GuiColorPicker(Rectangle bounds, char8* text, Color color);                        // Color Picker control (multiple color controls)
    public static Color GuiColorPanel(Rectangle bounds, char8* text, Color color);                         // Color Panel control
    public static float GuiColorBarAlpha(Rectangle bounds, char8* text, float alpha);                      // Color Bar Alpha control
    public static float GuiColorBarHue(Rectangle bounds, char8* text, float value);                        // Color Bar Hue control*/

    // Styles loading functions
    /*public static void GuiLoadStyle(char8* fileName);              // Load style file over global style variable (.rgs)
    public static void GuiLoadStyleDefault();                       // Load style default over global style*/

    // Icons functionality
    /*public static char8* GuiIconText(int iconId, char8* text); // Get text with icon id prepended (if supported)*/

#if !RAYGUI_NO_ICONS
    /*public static void GuiDrawIcon(int iconId, int posX, int posY, int pixelSize, Color color);

    public static uint32 *GuiGetIcons();                      // Get full icons data pointer
    public static uint32 *GuiGetIconData(int iconId);             // Get icon bit data
    public static void GuiSetIconData(int iconId, uint32 *data);  // Set icon bit data
    public static void GuiSetIconScale(uint32 scale);             // Set icon scale (1 by default)

    public static void GuiSetIconPixel(int iconId, int x, int y);       // Set icon pixel value
    public static void GuiClearIconPixel(int iconId, int x, int y);     // Clear icon pixel value
    public static bool GuiCheckIconPixel(int iconId, int x, int y);     // Check icon pixel value*/

#if !RAYGUI_CUSTOM_ICONS
//----------------------------------------------------------------------------------
// Icons enumeration
//----------------------------------------------------------------------------------
enum GuiIconName
...
static
{
	static mixin RAYGUI_CLITERAL(var name)
	{
		name
	}
	...
	const uint32 RAYGUI_ICON_SIZE               = 16;         // Size of icons in pixels (squared)
	const uint32 RAYGUI_ICON_MAX_ICONS         	= 256;          // Maximum number of icons
	const uint32 RAYGUI_ICON_MAX_NAME_LENGTH    = 32;          // Maximum length of icon name id

	// Icons data is defined by bit array (every bit represents one pixel)
	// Those arrays are stored as uint32 data arrays, so,
	...
	const uint32 RAYGUI_ICON_DATA_ELEMENTS   = (RAYGUI_ICON_SIZE * RAYGUI_ICON_SIZE / 32);
	...
	// guiIcons size is by default: 256*(16*16/32) = 2048*4 = 8192 bytes = 8 KB
	//----------------------------------------------------------------------------------
	static uint32[RAYGUI_ICON_MAX_ICONS * RAYGUI_ICON_DATA_ELEMENTS] guiIcons = .(
...
#else
    const uint32 RAYGUI_ICON_SIZE             = 0;
#endif      // !RAYGUI_NO_ICONS && !RAYGUI_CUSTOM_ICONS

	const uint32 RAYGUI_MAX_CONTROLS          = 16;      // Maximum number of standard controls
	const uint32 RAYGUI_MAX_PROPS_BASE        = 16;      // Maximum number of standard properties
	const uint32 RAYGUI_MAX_PROPS_EXTENDED    = 8;     // Maximum number of extended properties
	...
	static Font guiFont = default;                // Gui current font (WARNING: highly coupled to raylib)
	static bool guiLocked = false;              // Gui lock state (no inputs processed)
	static float guiAlpha = 1.0f;               // Gui element transpacency on drawing
	
	static uint32 guiIconScale = 1;       // Gui icon default scale (if icons enabled)
	...
	// guiStyle size is by default: 16*(16 + 8) = 384*4 = 1536 bytes = 1.5 KB
	//----------------------------------------------------------------------------------
	static uint32[RAYGUI_MAX_CONTROLS*(RAYGUI_MAX_PROPS_BASE + RAYGUI_MAX_PROPS_EXTENDED)] guiStyle = default;
	...
	static int GetCharPressed();         // -- GuiTextBox(), GuiTextBoxMulti(), GuiValueBox()
	//-------------------------------------------------------------------------------
	...
	//-------------------------------------------------------------------------------
	static Font LoadFontEx(char8* fileName, int fontSize, int *fontChars, int glyphCount); // -- GuiLoadStyle()
	static Font GetFontDefault();                           // -- GuiLoadStyleDefault()
	...
	static char8* LoadFileText(char8* fileName);            // -- GuiLoadStyle()
	static char8* GetDirectoryPath(char8* filePath);  // -- GuiLoadStyle()
	
	static Vector2 MeasureTextEx(Font font, char8* text, float fontSize, float spacing);   // -- GetTextWidth(), GuiTextBoxMulti()
	static void DrawTextEx(Font font, char8* text, Vector2 position, float fontSize, float spacing, Color tint);  // -- GuiDrawText()
	//-------------------------------------------------------------------------------
	...
	static char8* TextFormat(char8* text, ...);               // Formatting of text with variables to 'embed'
	static char8* *TextSplit(char8* text, char delimiter, int *count);    // Split text into multiple strings
	static int TextToInteger(char8* text);         // Get integer value from text
	static int GetCodepoint(char8* text, int *bytesProcessed);     // Get next codepoint in a UTF-8 encoded text
	static char8* CodepointToUTF8(int codepoint, int *byteSize);   // Encode codepoint into UTF-8 text (char array size returned as parameter)
	...
	// Window Box control
	public static bool GuiWindowBox(Rectangle bounds, char8* title)
	{
		var bounds;

	    // Window title bar height (including borders)
	    // NOTE: This define is also used by GuiMessageBox() and GuiTextInputBox()
	    const uint32 RAYGUI_WINDOWBOX_STATUSBAR_HEIGHT = 24;
	
	    //GuiState state = guiState;
	    bool clicked = false;
	
	    let statusBarHeight = RAYGUI_WINDOWBOX_STATUSBAR_HEIGHT;
	    ...
	    Rectangle closeButtonRec = .(statusBar.x + statusBar.width - GuiGetStyle(GuiControl.STATUSBAR.Underlying, GuiControlProperty.BORDER_WIDTH.Underlying) - 20,
	...
	// Panel control
	public static void GuiPanel(Rectangle bounds, char8* text)
	{
	...
	public static Rectangle GuiScrollPanel(Rectangle bounds, char8* text, Rectangle content, Vector2 *scroll)
	{
	    GuiState state = guiState;
	...
	{ (float)((GuiGetStyle(LISTVIEW, SCROLLBAR_SIDE) == SCROLLBAR_LEFT_SIDE)? (float)bounds.x + GuiGetStyle(DEFAULT, BORDER_WIDTH) : (float)bounds.x + bounds.width - verticalScrollBarWidth - GuiGetStyle(DEFAULT, BORDER_WIDTH)), (float)bounds.y + GuiGetStyle(DEFAULT, BORDER_WIDTH), (float)verticalScrollBarWidth, (float)bounds.height - horizontalScrollBarWidth - 2*GuiGetStyle(DEFAULT, BORDER_WIDTH) };
	...
	{
	        Rectangle corner =
	{ (GuiGetStyle(LISTVIEW, SCROLLBAR_SIDE) == SCROLLBAR_LEFT_SIDE)? (bounds.x + GuiGetStyle(DEFAULT, BORDER_WIDTH) + 2) : (horizontalScrollBar.x + horizontalScrollBar.width + 2), verticalScrollBar.y + verticalScrollBar.height + 2, (float)horizontalScrollBarWidth - 4, (float)verticalScrollBarWidth - 4 };
	...
	// Label control
	public static void GuiLabel(Rectangle bounds, char8* text)
	{
	    GuiState state = guiState;
	...
	public static bool GuiButton(Rectangle bounds, char8* text)
	{
	    GuiState state = guiState;
	    bool pressed = false;
	...
	public static bool GuiLabelButton(Rectangle bounds, char8* text)
	{
	    GuiState state = guiState;
	    bool pressed = false;
	...
	public static bool GuiToggle(Rectangle bounds, char8* text, bool active)
	{
	    GuiState state = guiState;
	...
	public static int GuiToggleGroup(Rectangle bounds, char8* text, int active)
	{
	    ...
	    int rows[RAYGUI_TOGGLEGROUP_MAX_ITEMS] =
	{ 0 };
	    int itemCount = 0;
	    char8* *items = GuiTextSplit(text, &itemCount, rows);
	...
	public static bool GuiCheckBox(Rectangle bounds, char8* text, bool checked)
	{
	    GuiState state = guiState;
	...
	public static int GuiComboBox(Rectangle bounds, char8* text, int active)
	{
	    GuiState state = guiState;
	...
	{ (float)bounds.x + bounds.width + GuiGetStyle(COMBOBOX, COMBO_BUTTON_SPACING),
	    ...
	    int itemCount = 0;
	    char8* *items = GuiTextSplit(text, &itemCount, NULL);
	...
	public static bool GuiDropdownBox(Rectangle bounds, char8* text, int *active, bool editMode)
	{
	    GuiState state = guiState;
	...
	public static bool GuiTextBox(Rectangle bounds, char8* text, int textSize, bool editMode)
	{
	    GuiState state = guiState;
	    bool pressed = false;
	       ...
	       
	{
	            ...
	            int byteSize = 0;
	            char8* textUTF8 = CodepointToUTF8(key, &byteSize);
	...
	public static bool GuiSpinner(Rectangle bounds, char8* text, int *value, int minValue, int maxValue, bool editMode)
	{
	    GuiState state = guiState;
	...
	{ (float)bounds.x, (float)bounds.y, (float)GuiGetStyle(SPINNER, SPIN_BUTTON_WIDTH), (float)bounds.height };
	    Rectangle rightButtonBound =
	{ (float)bounds.x + bounds.width - GuiGetStyle(SPINNER, SPIN_BUTTON_WIDTH), (float)bounds.y, (float)GuiGetStyle(SPINNER, SPIN_BUTTON_WIDTH), (float)bounds.height };
	...
	public static bool GuiValueBox(Rectangle bounds, char8* text, int *value, int minValue, int maxValue, bool editMode)
	{
	   ...
	   
	{
	        ...
	        Rectangle cursor =
	{ bounds.x + GetTextWidth(textValue)/2 + bounds.width/2 + 2, bounds.y + 2*GuiGetStyle(VALUEBOX, BORDER_WIDTH), 4, bounds.height - 4*GuiGetStyle(VALUEBOX, BORDER_WIDTH) };
	...
	public static bool GuiTextBoxMulti(Rectangle bounds, char8* text, int textSize, bool editMode)
	{
	    GuiState state = guiState;
	    bool pressed = false;
	    ...
	    Rectangle textAreaBounds =
	{
	    ...
	    Rectangle cursor =
	{ 0, -1, 4, (float)GuiGetStyle(DEFAULT, TEXT_SIZE) + 2 };
	       ...
	       
	{
	            ...
	            int byteSize = 0;
	            char8* textUTF8 = CodepointToUTF8(codepoint, &byteSize);
	               ...
	               
	{
	                    ...
	                    char8* charEncoded = CodepointToUTF8(codepoint, &charUTF8Length);
	   ...
	   
	{
	    ...
	    Vector2 cursorPos =
	{ textAreaBounds.x, textAreaBounds.y };
	...
	public static float GuiSliderPro(Rectangle bounds, char8* textLeft, char8* textRight, float value, float minValue, float maxValue, int sliderWidth)
	{
	    GuiState state = guiState;
	...
	public static float GuiSlider(Rectangle bounds, char8* textLeft, char8* textRight, float value, float minValue, float maxValue)
	{
	    return GuiSliderPro(bounds, textLeft, textRight, value, minValue, maxValue, GuiGetStyle(SLIDER, SLIDER_WIDTH));
	}

	// Slider Bar control extended, returns selected value
	public static float GuiSliderBar(Rectangle bounds, char8* textLeft, char8* textRight, float value, float minValue, float maxValue)
	{
	    return GuiSliderPro(bounds, textLeft, textRight, value, minValue, maxValue, 0);
	}

	// Progress Bar control extended, shows current progress value
	public static float GuiProgressBar(Rectangle bounds, char8* textLeft, char8* textRight, float value, float minValue, float maxValue)
	{
	    GuiState state = guiState;
	...
	// Status Bar control
	public static void GuiStatusBar(Rectangle bounds, char8* text)
	{
	    GuiState state = guiState;
	...
	public static void GuiDummyRec(Rectangle bounds, char8* text)
	{
	    GuiState state = guiState;
	...
	// List View control
	public static int GuiListView(Rectangle bounds, char8* text, int *scrollIndex, int active)
	{
	    int itemCount = 0;
	    char8* *items = NULL;
	...
	public static int GuiListViewEx(Rectangle bounds, char8* *text, int count, int *focus, int *scrollIndex, int active)
	{
	    GuiState state = guiState;
	...
	// Color Panel control
	public static Color GuiColorPanel(Rectangle bounds, char8* text, Color color)
	{
	    const Color colWhite =
	{ 255, 255, 255, 255 };
	    const Color colBlack =
	{ 0, 0, 0, 255 };
	           ...
	           
	{
	                ...
	                Vector2 colorPick =
	{ pickerSelector.x - bounds.x, pickerSelector.y - bounds.y };
	   ...
	   
	{
	        ...
	        Rectangle selector =
	{ pickerSelector.x - GuiGetStyle(COLORPICKER, COLOR_SELECTOR_SIZE)/2, pickerSelector.y - GuiGetStyle(COLORPICKER, COLOR_SELECTOR_SIZE)/2, (float)GuiGetStyle(COLORPICKER, COLOR_SELECTOR_SIZE), (float)GuiGetStyle(COLORPICKER, COLOR_SELECTOR_SIZE) };
	...
	public static float GuiColorBarAlpha(Rectangle bounds, char8* text, float alpha)
	{
	    ...
	    GuiState state = guiState;
	    Rectangle selector =
	{ (float)bounds.x + alpha*bounds.width - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_HEIGHT)/2, (float)bounds.y - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW), (float)GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_HEIGHT), (float)bounds.height + GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW)*2 };
	...
	{
	                Rectangle check =
	{ bounds.x + x*RAYGUI_COLORBARALPHA_CHECKED_SIZE, bounds.y + y*RAYGUI_COLORBARALPHA_CHECKED_SIZE, RAYGUI_COLORBARALPHA_CHECKED_SIZE, RAYGUI_COLORBARALPHA_CHECKED_SIZE };
	...
	public static float GuiColorBarHue(Rectangle bounds, char8* text, float hue)
	{
	    GuiState state = guiState;
	    Rectangle selector =
	{ (float)bounds.x - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW), (float)bounds.y + hue/360.0f*bounds.height - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_HEIGHT)/2, (float)bounds.width + GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW)*2, (float)GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_HEIGHT) };
	   ...
	   
	{
	        ...
	        DrawRectangleGradientV((int)bounds.x, (int)(bounds.y), (int)bounds.width, (int)ceilf(bounds.height/6),  Fade(RAYGUI_CLITERAL(Color)
	{ 255, 0, 0, 255 }, guiAlpha), Fade(RAYGUI_CLITERAL(Color)
	{ 255, 255, 0, 255 }, guiAlpha));
	        DrawRectangleGradientV((int)bounds.x, (int)(bounds.y + bounds.height/6), (int)bounds.width, (int)ceilf(bounds.height/6), Fade(RAYGUI_CLITERAL(Color)
	{ 255, 255, 0, 255 }, guiAlpha), Fade(RAYGUI_CLITERAL(Color)
	{ 0, 255, 0, 255 }, guiAlpha));
	        DrawRectangleGradientV((int)bounds.x, (int)(bounds.y + 2*(bounds.height/6)), (int)bounds.width, (int)ceilf(bounds.height/6), Fade(RAYGUI_CLITERAL(Color)
	{ 0, 255, 0, 255 }, guiAlpha), Fade(RAYGUI_CLITERAL(Color)
	{ 0, 255, 255, 255 }, guiAlpha));
	        DrawRectangleGradientV((int)bounds.x, (int)(bounds.y + 3*(bounds.height/6)), (int)bounds.width, (int)ceilf(bounds.height/6), Fade(RAYGUI_CLITERAL(Color)
	{ 0, 255, 255, 255 }, guiAlpha), Fade(RAYGUI_CLITERAL(Color)
	{ 0, 0, 255, 255 }, guiAlpha));
	        DrawRectangleGradientV((int)bounds.x, (int)(bounds.y + 4*(bounds.height/6)), (int)bounds.width, (int)ceilf(bounds.height/6), Fade(RAYGUI_CLITERAL(Color)
	{ 0, 0, 255, 255 }, guiAlpha), Fade(RAYGUI_CLITERAL(Color)
	{ 255, 0, 255, 255 }, guiAlpha));
	        DrawRectangleGradientV((int)bounds.x, (int)(bounds.y + 5*(bounds.height/6)), (int)bounds.width, (int)(bounds.height/6), Fade(RAYGUI_CLITERAL(Color)
	{ 255, 0, 255, 255 }, guiAlpha), Fade(RAYGUI_CLITERAL(Color)
	{ 255, 0, 0, 255 }, guiAlpha));
	    }
	...
	public static Color GuiColorPicker(Rectangle bounds, char8* text, Color color)
	{
	...
	{ (float)bounds.x + bounds.width + GuiGetStyle(COLORPICKER, HUEBAR_PADDING), (float)bounds.y, (float)GuiGetStyle(COLORPICKER, HUEBAR_WIDTH), (float)bounds.height };
	    //Rectangle boundsAlpha =
	{ bounds.x, bounds.y + bounds.height + GuiGetStyle(COLORPICKER, BARS_PADDING), bounds.width, GuiGetStyle(COLORPICKER, BARS_THICK) };
	...
	// Message Box control
	public static int GuiMessageBox(Rectangle bounds, char8* title, char8* message, char8* buttons)
	{
	    ...
	    int buttonCount = 0;
	    char8* *buttonsText = GuiTextSplit(buttons, &buttonCount, NULL);
	...
	public static int GuiTextInputBox(Rectangle bounds, char8* title, char8* message, char8* buttons, char8* text, int textMaxSize, int *secretViewActive)
	{
	...
	public static Vector2 GuiGrid(Rectangle bounds, char8* text, float spacing, int subdivs)
	{
	...
	{
	                    Rectangle lineV =
	{ bounds.x + spacing*i/subdivs, bounds.y, 1, bounds.height };
	...
	{
	                    Rectangle lineH =
	{ bounds.x, bounds.y + spacing*i/subdivs, bounds.width, 1 };
	...
	public static void GuiLoadStyle(char8* fileName)
	{
	                       ...
	                       
	{
	                            ...
	                            char8* charValues = LoadFileText(charmapFileName);
	                            if (charValues != NULL)
	                           
	{
	                                int glyphCount = 0;
	                                char8* *chars = TextSplit(charValues, '\n', &glyphCount);
	           ...
	           
	{
	                ...
	                fread(&propertyValue, 1, sizeof(uint32), rgsFile);
	               ...
	               
	{
	                    ...
	                    unsigned char8* compData = (unsigned char8* )RAYGUI_MALLOC(fontImageCompSize);
	               ...
	               
	{
	                    ...
	                    imFont.data = (unsigned char8* )RAYGUI_MALLOC(fontImageUncompSize);
	...
	public static char8* GuiIconText(int iconId, char8* text)
	{
	#if defined(RAYGUI_NO_ICONS)
	    return NULL;
	#else
	...
	public static uint32 *GuiGetIcons()
	{ return guiIcons; }
	...
	public static char8* *GuiLoadIcons(char8* fileName, bool loadIconsName)
	{
	    ...
	    //
	{
	    ...
	    // Icons data: One bit per pixel, stored as uint32 array (depends on icon size)
	    // S*S pixels/32bit per uint32 = K uint32 per icon
	    // foreach (icon)
	    //
	{
	    //   ...   | K       | uint32 | Icon Data
	...
	{
	                guiIconsName = (char8* *)RAYGUI_MALLOC(iconCount*sizeof(char8* *));
	                for (int i = 0; i < iconCount; i++)
	               
	{
	                    guiIconsName[i] = (char8* )RAYGUI_MALLOC(RAYGUI_ICON_MAX_NAME_LENGTH);
	            ...
	            fread(guiIcons, iconCount*(iconSize*iconSize/32), sizeof(uint32), rgiFile);
	        }
	...
	// Get icon bit data
	// NOTE: Bit data array grouped as uint32 (RAYGUI_ICON_SIZE*RAYGUI_ICON_SIZE/32 elements)
	public static uint32 *GuiGetIconData(int iconId)
	{
	    static uint32 iconData[RAYGUI_ICON_DATA_ELEMENTS] =
	{ 0 };
	    memset(iconData, 0, RAYGUI_ICON_DATA_ELEMENTS*sizeof(uint32));
	
	    if (iconId < RAYGUI_ICON_MAX_ICONS) memcpy(iconData, &guiIcons[iconId*RAYGUI_ICON_DATA_ELEMENTS], RAYGUI_ICON_DATA_ELEMENTS*sizeof(uint32));
	
	    return iconData;
	}

	// Set icon bit data
	// NOTE: Data must be provided as uint32 array (RAYGUI_ICON_SIZE*RAYGUI_ICON_SIZE/32 elements)
	public static void GuiSetIconData(int iconId, uint32 *data)
	{
	    if (iconId < RAYGUI_ICON_MAX_ICONS) memcpy(&guiIcons[iconId*RAYGUI_ICON_DATA_ELEMENTS], data, RAYGUI_ICON_DATA_ELEMENTS*sizeof(uint32));
	}

	// Set icon scale (1 by default)
	public static void GuiSetIconScale(uint32 scale)
	{
	...
	public static void GuiSetIconPixel(int iconId, int x, int y)
	{
	    ...
	    // For example, in case of 16x16 pixels, every 2 lines fit in one uint32 data element
	    BIT_SET(guiIcons[iconId*RAYGUI_ICON_DATA_ELEMENTS + y/(sizeof(uint32)*8/RAYGUI_ICON_SIZE)], x + (y%(sizeof(uint32)*8/RAYGUI_ICON_SIZE)*RAYGUI_ICON_SIZE));
	}
	...
	public static void GuiClearIconPixel(int iconId, int x, int y)
	{
	    ...
	    // This logic works for any RAYGUI_ICON_SIZE pixels icons,
	    // For example, in case of 16x16 pixels, every 2 lines fit in one uint32 data element
	    BIT_CLEAR(guiIcons[iconId*RAYGUI_ICON_DATA_ELEMENTS + y/(sizeof(uint32)*8/RAYGUI_ICON_SIZE)], x + (y%(sizeof(uint32)*8/RAYGUI_ICON_SIZE)*RAYGUI_ICON_SIZE));
	}
	...
	static int GetTextWidth(char8* text)
	{
	...
	static char8* GetTextIcon(char8* text, int *iconId)
	{
	#if !defined(RAYGUI_NO_ICONS)
	    *iconId = -1;
	...
	static void GuiDrawText(char8* text, Rectangle bounds, int alignment, Color tint)
	{
	...
	static char8* *GuiTextSplit(char8* text, int *count, int *textRow)
	{
	    ...
	    static char8* result[RAYGUI_TEXTSPLIT_MAX_ITEMS] =
	{ NULL };
	...
	{ (float)bounds.x + GuiGetStyle(SCROLLBAR, BORDER_WIDTH), (float)bounds.y + GuiGetStyle(SCROLLBAR, BORDER_WIDTH), (float)spinnerSize, (float)spinnerSize };
	
	    if (isVertical)
	   
	{
	        arrowDownRight = RAYGUI_CLITERAL(Rectangle)
	{ (float)bounds.x + GuiGetStyle(SCROLLBAR, BORDER_WIDTH), (float)bounds.y + bounds.height - spinnerSize - GuiGetStyle(SCROLLBAR, BORDER_WIDTH), (float)spinnerSize, (float)spinnerSize };
	        scrollbar = RAYGUI_CLITERAL(Rectangle)
	{ bounds.x + GuiGetStyle(SCROLLBAR, BORDER_WIDTH) + GuiGetStyle(SCROLLBAR, SCROLL_PADDING), arrowUpLeft.y + arrowUpLeft.height, bounds.width - 2*(GuiGetStyle(SCROLLBAR, BORDER_WIDTH) + GuiGetStyle(SCROLLBAR, SCROLL_PADDING)), bounds.height - arrowUpLeft.height - arrowDownRight.height - 2*GuiGetStyle(SCROLLBAR, BORDER_WIDTH) };
	...
	{ (float)bounds.x + GuiGetStyle(SCROLLBAR, BORDER_WIDTH) + GuiGetStyle(SCROLLBAR, SCROLL_SLIDER_PADDING), (float)scrollbar.y + (int)(((float)(value - minValue)/range)*(scrollbar.height - sliderSize)), (float)bounds.width - 2*(GuiGetStyle(SCROLLBAR, BORDER_WIDTH) + GuiGetStyle(SCROLLBAR, SCROLL_SLIDER_PADDING)), (float)sliderSize };
	    }
	    else
	   
	{
	        arrowDownRight = RAYGUI_CLITERAL(Rectangle)
	{ (float)bounds.x + bounds.width - spinnerSize - GuiGetStyle(SCROLLBAR, BORDER_WIDTH), (float)bounds.y + GuiGetStyle(SCROLLBAR, BORDER_WIDTH), (float)spinnerSize, (float)spinnerSize };
	        scrollbar = RAYGUI_CLITERAL(Rectangle)
	{ arrowUpLeft.x + arrowUpLeft.width, bounds.y + GuiGetStyle(SCROLLBAR, BORDER_WIDTH) + GuiGetStyle(SCROLLBAR, SCROLL_PADDING), bounds.width - arrowUpLeft.width - arrowDownRight.width - 2*GuiGetStyle(SCROLLBAR, BORDER_WIDTH), bounds.height - 2*(GuiGetStyle(SCROLLBAR, BORDER_WIDTH) + GuiGetStyle(SCROLLBAR, SCROLL_PADDING)) };
	        ...
	        slider = RAYGUI_CLITERAL(Rectangle)
	{ (float)scrollbar.x + (int)(((float)(value - minValue)/range)*(scrollbar.width - sliderSize)), (float)bounds.y + GuiGetStyle(SCROLLBAR, BORDER_WIDTH) + GuiGetStyle(SCROLLBAR, SCROLL_SLIDER_PADDING), (float)sliderSize, (float)bounds.height - 2*(GuiGetStyle(SCROLLBAR, BORDER_WIDTH) + GuiGetStyle(SCROLLBAR, SCROLL_SLIDER_PADDING)) };
	    }
	...
	{ arrowUpLeft.x + arrowUpLeft.width, arrowUpLeft.y, scrollbar.width, bounds.height - 2*GuiGetStyle(SCROLLBAR, BORDER_WIDTH) };
	...
	{
	                    Rectangle scrollArea =
	{ arrowUpLeft.x, arrowUpLeft.y+arrowUpLeft.height, bounds.width - 2*GuiGetStyle(SCROLLBAR, BORDER_WIDTH),  scrollbar.height };
...
static Color Fade(Color color, float alpha)
{
    ...
    Color result =
{ color.r, color.g, color.b, (unsigned char)(255.0f*alpha) };

    return result;
}

// Formatting of text with variables to 'embed'
static char8* TextFormat(char8* text, ...)
{
...
char8* *TextSplit(char8* text, char delimiter, int *count)
{
    // NOTE: Current implementation returns a copy of the provided string with '\0' (string end delimiter)
    // inserted between strings defined by "delimiter" parameter. No memory is dynamically allocated,
    // all used memory is static... it has some limitations:
...
static int TextToInteger(char8* text)
{
    int value = 0;
    int sign = 1;
...
static char8* CodepointToUTF8(int codepoint, int *byteSize)
{
    static char utf8[6] =
{ 0 };
    int size = 0;
...
static int GetCodepoint(char8* text, int *bytesProcessed)
{
/*
...
{ *bytesProcessed = 2; return code; } // Unexpected sequence
...
{ *bytesProcessed = 3; return code; } // Unexpected sequence
...
{ *bytesProcessed = 4; return code; }  // Unexpected sequence
